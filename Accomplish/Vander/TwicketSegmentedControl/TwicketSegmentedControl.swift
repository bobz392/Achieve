...
open class TwicketSegmentedControl: UIControl {
    ...
    open func move(to index: Int, animation: Bool = true) {
        let correctOffset = center(at: index)
        animate(to: correctOffset, animation: animation)
    ...
    fileprivate func animate(to position: CGFloat, animation: Bool) {
        if animation {
